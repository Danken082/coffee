...
return redirect()->to('trialnotif2');
    ...
    public function savePOSOrders()
    {
       ...
       foreach ($requestData as $item) {
           ...
           $savedData[] = [
            ...
            if ($productsize === "Large" && $productCategory === "Hot Coffee" && $productID >= 32 && $productID <= 41) {
            ...
            } elseif ($productsize === "Large" && $productCategory === "Iced Coffee" && $productID >= 42 && $productID <= 44) {
            ...
            if ($productsize === "Regular" && $productCategory === "Hot Coffee" && $productID >= 32 && $productID <= 41) {
                $HotCoffeeStock = $RCHC['stocks'] - $totalquantity;
                $WhiteStrawStock = $WS['stocks'] - $totalquantity;

                $this->raw->where('rawID', $RCHC['rawID'])->set(['stocks' => $HotCoffeeStock])->update();
                $this->raw->where('rawID', $WS['rawID'])->set(['stocks' => $WhiteStrawStock])->update();
            } elseif ($productsize === "Regular" && $productCategory === "Iced Coffee" && $productID >= 42 && $productID <= 44) {
            ...
            if (($productID >= 1 && $productID <= 3) || ($productID >= 11 && $productID <= 16)) {
                $GrapesPaperStock = $GP['stocks'] - $totalquantity;

                $this->raw->where('rawID', $GP['rawID'])->set(['stocks' => $GrapesPaperStock])->update();
            }

            if ($DineTake == "Take Out" && $productCategory === "Salad" && $productID >= 26 && $productID <= 29) {
                $SaladStock = $Salad['stocks'] - $totalquantity;

                $this->raw->where('rawID', $Salad['rawID'])->set(['stocks' => $SaladStock])->update();
            }

            if ($DineTake == "Take Out" && $productCategory === "Sandwich" && $productID >= 11 && $productID <= 16) {
                $SandwichStock = $SW['stocks'] - $totalquantity;

                $this->raw->where('rawID', $SW['rawID'])->set(['stocks' => $SandwichStock])->update();
            }
  ...
  $data['order'] = $this->payment->select('order.orderID,order.barcode, user.UserID, product_tbl.prod_id, order.CustomerID, order.ProductID, order.total, order.orderStatus, 
    ...
    public function viewToAcceptorders($barcode)
    {
        ...
        //for Multi Data
       $data['barcode'] =  $this->order->select('order.orderID, order.CustomerID, order.ProductID, order.paymentStatus,order.ImagePayment, 
        order.orderType, order.orderDate, order.total, order.quantity, order.size, order.barcode, order.orderStatus,
        product_tbl.prod_id, product_tbl.prod_name, product_tbl.prod_quantity, product_tbl.prod_mprice, 
        product_tbl.prod_lprice, product_tbl.prod_desc, product_tbl.prod_img, product_tbl.prod_categ, product_tbl.prod_code,
        user.UserID, user.LastName, user.FirstName, user.email, user.address, user.ContactNo')
