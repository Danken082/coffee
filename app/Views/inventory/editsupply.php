    ...
    <div class="col-md-12">
        <div class="card my-4">
            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2 d-flex justify-content-between">
                ...
                <div>
                    <a href="<?= base_url('/inventorysupply')?>" class="btn btn-info">BACK</a>
                </div>
            </div>
            <div class="card-body px-4 pb-3">
                <form action="<?= base_url('/updatesupply/'. $supply['rawID'])?>" class="row g-3" method="POST" enctype="multipart/form-data">
                    <div class="input-group input-group-outline my-3">
                        ...
                        <input type="text" class="form-control" id="name" name="name" value="<?=$supply['name'] ?>" placeholder="Name">
                    </div>
                    <div class="input-group input-group-outline my-3">
                        <label for="stocks" class="form-label"></label>
                        <input type="number" class="form-control" id="stocks" name="stocks" min="1" value="<?=$supply['stocks'] ?>" placeholder="Stocks">
                    </div>
                    <div class="input-group input-group-outline my-3">
                        <label for="barcode" class="form-label"></label>
                        <input type="text" disabled class="form-control" id="barcode" name="barcode" value="<?=$supply['barcode'] ?>" placeholder="Barcode">
                    </div>
                    <div class="input-group input-group-outline my-3 text-center" style="display: flex; justify-content: center;">
                        <button type="submit" class="btn btn-success" style="background-color: #1167b1; color: white;">Update Supplies</button>
                    </div>
                </form>
            </div>
        </div>
